...
typedef struct dpc
{
    ...
    // Returns the same DPC object if the handler doesn't free it
    ...
    struct dpc*(*handler)(struct dpc* dpc, void* userdata);
    void* userdata;
    struct cpu_local* cpu;
} dpc;

OBOS_EXPORT dpc* CoreH_AllocateDPC(obos_status* status);
OBOS_EXPORT obos_status CoreH_InitializeDPC(dpc* dpc, struct dpc*(*handler)(struct dpc* obj, void* userdata), thread_affinity affinity);
