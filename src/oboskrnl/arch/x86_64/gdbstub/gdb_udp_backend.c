...
static obos_status write_sync(dev_desc desc, const void* buf, size_t blkCount, size_t blkOffset, size_t* nBlkWritten)
{
    ...
    udp_header* hdr = Allocate(OBOS_KernelAllocator, sizeof(udp_header)+blkCount, nullptr);
    hdr->dest_port = host_to_be16(hnd->client_port);
    hdr->src_port = host_to_be16(hnd->bound_port->port);
    hdr->length = host_to_be16(sizeof(udp_header)+blkCount);
    hdr->chksum = 0;
    memcpy(hdr+1, buf, blkCount);

    shared_ptr *data = ZeroAllocate(OBOS_KernelAllocator, 1, sizeof(shared_ptr), nullptr);
    OBOS_SharedPtrConstructSz(data, hdr, sizeof(*hdr)+blkCount);
    data->free = OBOS_SharedPtrDefaultFree;
    data->freeUdata = OBOS_KernelAllocator;
    data->onDeref = NetFreeSharedPtr;
    NetH_SendIPv4Packet(hnd->interface, hnd->table_ent, hnd->client, 0x11, 128, 0, OBOS_SharedPtrCopy(data));
...
obos_status Kdbg_ConnectionInitializeUDP(gdb_connection* conn, uint16_t bind_port, vnode* interface)
{
    ...
    udp_port *bound_port = ZeroAllocate(OBOS_KernelAllocator, 1, sizeof(udp_port), nullptr);
    bound_port->port = bind_port;
    bound_port->recv_event = EVENT_INITIALIZE(EVENT_NOTIFICATION);
    handle->bound_port = bound_port;
    RB_INSERT(udp_port_tree, &interface->net_tables->udp_ports, bound_port);
