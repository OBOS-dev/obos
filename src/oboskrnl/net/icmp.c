...
obos_status Net_ICMPv4ParameterProblem(tables* tbl, const ip_header* ip_hdr, const frame* raw_frame, uint8_t offset)
{
    OBOS_UNUSED(ip_hdr);
    if (!tbl || !ip_hdr || !raw_frame)
        return OBOS_STATUS_INVALID_ARGUMENT;
    ...
    obos_status status = Net_FormatICMPv4Packet((icmp_header**)&hdr, ip_hdr, be16_to_host(ip_hdr->packet_length), ICMPv4_TYPE_PARAMETER_PROBLEM, 0, host_to_be32((uint32_t)offset));
    if (obos_is_error(status))
        goto cleanup;
