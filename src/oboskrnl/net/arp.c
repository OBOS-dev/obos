...
obos_status Net_ARPReceiveFrame(frame* data)
{
    ...
    ip_addr our_ip = { .addr=(uint32_t)data->source_vn->data };
    switch (be16_to_host(hdr->opcode))
    {
        case ARP_REQUEST:
        {
            ...
            if (our_ip.addr == target_ip.addr)
            {
                ...
                status = Net_FormatARPReplyIPv4(&reply, data->source_mac_address, our_ip, sender_mac, sender_ip);
                ...
                status = Net_FormatEthernet2Packet(&eth2,
                                          ...
                                          (mac_address*)sender_mac, &data->source_mac_address,
                                          ETHERNET2_TYPE_ARP,
                                          &frame_size);
                if (obos_is_error(status))
                {
                ...
                // Send the reply
                status = data->source_vn->un.device->driver->header.ftable.write_sync(data->source_vn->desc, eth2, frame_size, 0, nullptr);
