...
#define SHRINK_AT   (fixedpt_xdiv(fixedpt_fromint(10), fixedpt_fromint(100))) /* 60% */

#define percentage_to_fixedpt(percent) (fixedpt_xdiv(fixedpt_fromint(percent), fixedpt_fromint(100)))

#ifndef HASHMAP_LOAD_FACTOR
#define HASHMAP_LOAD_FACTOR GROW_AT
#endif

static void *(*__malloc)(size_t) = nullptr;
static void *(*__realloc)(void *, size_t) = nullptr;
static void (*__free)(void *) = nullptr;
...
static fixedptd clamp_load_factor(fixedptd factor, fixedptd default_factor) {
    ...
    //     default_factor : (factor < 0.50) ?
    //      0.50 : factor > 0.95 ?
    //      0.95 : factor;
    // TODO(oberrow): Check for NaN (if possible).
    return (factor < percentage_to_fixedpt(50)) ?
         percentage_to_fixedpt(50) : factor > percentage_to_fixedpt(95) ?
         percentage_to_fixedpt(95) : factor;
}

void hashmap_set_load_factor(struct hashmap *map, fixedptd factor) {
    factor = clamp_load_factor(factor, map->loadfactor / 100);
    map->loadfactor = fixedpt_toint(fixedpt_xmul(factor, fixedpt_fromint(100)));
...
struct hashmap *hashmap_new_with_allocator(void *(*_malloc)(size_t), 
    ...
    void *udata)
{
    _malloc = _malloc ? _malloc : __malloc ? __malloc : nullptr;
    _realloc = _realloc ? _realloc : __realloc ? __realloc : nullptr;
    _free = _free ? _free : __free ? __free : nullptr;
    size_t ncap = 16;
    if (cap < ncap) {
        cap = ncap;
    } else {
...
// Param `elfree` is a function that frees a specific item. This should be nullptr
...
struct hashmap *hashmap_new(size_t elsize, size_t cap, uint64_t seed0, 
    ...
    void *udata)
{
    return hashmap_new_with_allocator(nullptr, nullptr, nullptr, elsize, cap, seed0, 
...
void hashmap_clear(struct hashmap *map, bool update_cap) {
    ...
    map->mask = map->nbuckets-1;
    map->growat = map->nbuckets * (map->loadfactor / 100);
    map->shrinkat = fixedpt_toint(fixedpt_xmul(map->nbuckets, SHRINK_AT));
}
...
// memory then nullptr is returned and hashmap_oom() returns true.
...
// hashmap_probe returns the item in the bucket at position or nullptr if an item
...
OBOS_NO_UBSAN bool hashmap_iter(struct hashmap *map, size_t *i, void **item) {
...
static void all(void) {
    int seed = getenv("SEED")?atoi(getenv("SEED")):time(nullptr);
    ...
    while (!(map = hashmap_new(sizeof(int), 0, seed, seed, 
                               hash_int, compare_ints_udata, nullptr, nullptr))) {}
    ...
    while (!(map = hashmap_new(sizeof(char*), 0, seed, seed,
                               hash_str, compare_strs, free_str, nullptr)));
